...
1. For training images for **segmentation of mitochondria (STEP 1)** we applied empanada modul [*Finetune a model*](https://empanada.readthedocs.io/en/latest/tutorials/finetune_tutorial.html#finetune-tutorial). We created two different models, the first one for original data acquired by Jeol 1400+ electron microscope, the second for scanned images from older electron microscope photographs. In both case we finetuned *MitoNet_v1* model, which is delivered with empanada installation. We finetuned *4 last layers* of this model, *Patch size in pixels* = 512 and for *30 epochs**. Images and their masks, both 8bit, exported from Labkit and prepared as described [below](https://github.com/LMCF-IMG/Analysis_Mitochondrial_Ultrastructure_and_Morphology/blob/main/README.md#preparation-of-image-data-for-training) in details, were organized as shown [here](https://empanada.readthedocs.io/en/latest/tutorials/finetune_tutorial.html#finetuning-the-model).

2. For creation of the panoptic model for **segmentation of cristae (STEP 2)** with recognizing **12 morphological classes**. For this traning we used another modul [*Train a model*](https://empanada.readthedocs.io/en/latest/tutorials/train_panop.html#training-the-model). We applied *Label divisor* = 1000 - see prep of cristae image data below, model architecture *PanopticDeepLab*, *Finetunable layers* = all, *CEM pretrained weights*** checked, *Patch size in pixels* = 512 and *200 epochs**. In this case, organizing data for training did not work as simply as in the case of mitochondria. Both images (8-bit) and masks (32-bit) had to be uploaded to empanada and saved using the [*Save finetune/training patches*](https://empanada.readthedocs.io/en/latest/modules/_save_patches.html#save-finetune-training-patches) modul. This simple but essential information for training the panoptic model is not documented anywhere. Please, see also comments about not straightforward [*cristae data prep*](https://github.com/LMCF-IMG/Analysis_Mitochondrial_Ultrastructure_and_Morphology/blob/main/README.md#additional-comments-to-cristae-data-prep-for-panoptic-segmentation-training) for other informations. Obtaining a model for panoptic segmentation of cristae was not easy, even the guy from NIH/NCI, one of empanada creators, commented: "*That’s great to hear that it worked at the end. Yeah, the model was trained for bag mitochondria’s, its exciting to hear that its doing good for cristae as well. (email August 18, 2025)*".
...
- **STEP 1** Mitochondria segmentation models:
  - [**Jeol 1400+ electron microscope data model**](https://owncloud.cesnet.cz/index.php/s/GRxauIeDbnSLiZV)
    ...
    - Comparison with original [MitoNet_v1 model](https://www.ebi.ac.uk/empiar/EMPIAR-11037/) trained using circa 135K mitochondrial instances:  
      ...
      *The higher IoU value, the better. IoU range = <0,1>.*
...
I used [umetrix](https://pypi.org/project/umetrix/) project to evaluate the quality of the resulting trained models. It allows us to calculate basic segmentation metrics, e.g. IoU, confusion matrix. The project also allows us to evaluate a set of images in a directory in batch mode. The inputs to the algorithm are masks found by the trained model and those drawn manually in Labkit.
...
In the case of **cristae, however, it is different**. We defined twelve different basic morphological shapes of crists, see the accompanying poster. In Labkit, we therefore consistently used twelve different labels, see Fig. 2. Three problems here arose:
...
Fig. 2: Segmented cristae image (left), labeling in Labkit using 12 labels and spots (middle), exported mask with the spots that had to be manually removed (right).
